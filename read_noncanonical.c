...
void alarmHandler(
    int signal) // User-defined function to handle alarms (handler function)
...
void infoFrameRead(int fd, FILE *fptr) {
  ...
  while (!stop) {
    ...
    } // esperar até ter dados no buffer
      ...
      if (buf[0] == FLAG) { // caso receber FLAG final
        ...
        if (data_count <= 1) {
          ...
          if (BCC2 == data[data_count]) { // Verificar BCC2
            ...
            // unstuff
            unsigned char unstuffed_data[2 * BUF_SIZE]; // Buffer com espaço
                                                        // extra para stuffing
            int skip = 0;
...
int main(int argc, char *argv[]) {
  ...
  newtio.c_lflag = 0;
  newtio.c_cc[VTIME] = (cc_t)0.1; // Inter-character timer unused
